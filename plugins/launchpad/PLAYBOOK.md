...
**A secret on a command line is a secret in `ps`.** `security add-generic-password -w <value>` puts the value in the process's argv, readable by every user on the machine for as long as it runs — Apple's own usage text calls `-w` insecure. Feed `security -i` the command on stdin, with the value as `-X <hex>` (the interactive parser splits on whitespace, and hex carries a multi-line PEM or JSON verbatim), end the line with a newline or it never runs, and read the item back: `-i` exits with the status of its last command only. And never hand `security` a keychain path that does not exist — it was observed to exit 0 and write to the login keychain instead.
