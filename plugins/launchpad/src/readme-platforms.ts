import { PLATFORMS, SURFACE_LABELS, type Platform } from './platforms.js';
...
const surfaces = (p: Platform) => (p.archetypes ?? []).map(a => SURFACE_LABELS[a]).join(' + ');

export function renderPlatformsBlock(platforms: readonly Platform[] = PLATFORMS): string {
