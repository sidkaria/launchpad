...
/**
 * The Readiness Scorecard — the WHAT.
 *
 * Most people with an unshipped app do not know the list. Not "how do I
 * notarize" — they do not know notarization is *on* the list. They do not know
 * their release build is signed with the debug keystore, that losing that
 * keystore means never updating the app again, or that a store will reject
 * them for a missing privacy policy.
 *
 * So this names the list and scores every app against it. Each finding says
 * what it means and what happens if it is ignored, because a red cell with no
 * consequence attached teaches nothing.
 *
 * Everything here is DERIVED from state and files on disk. Nothing is asked of
 * the user and nothing is cached: a scorecard that can go stale is a scorecard
 * that lies.
 */

export type Grade = 'ok' | 'gap' | 'unknown' | 'n/a';

export interface Check {
  id: string;
  title: string;
  grade: Grade;
  /** What this means and what it costs. Present whenever the grade is not 'ok'. */
  detail: string;
  /** True when ignoring this can cost something unrecoverable. Sorts to the top. */
  critical?: boolean;
  /**
   * Set when this passed because the USER said so, not because launchpad saw
   * anything. Rendered differently on purpose: "you confirmed this in March" is
   * a different claim from "this is verified", and conflating them would make
   * the scorecard lie in the one place it was being scrupulous.
   */
  confirmedAt?: string;
  /** Can the user answer this one? True only where launchpad genuinely cannot. */
  answerable?: boolean;
}
...
export interface Scorecard {
  ...
  checks: Check[];
  /**
   * Progress over the checks launchpad can actually decide, 0-100.
   *
   * Kept for the dashboard's progress ring and nothing else — no human-facing
   * text quotes it. Two things were wrong with it as a headline.
   *
   * First, `unknown` used to sit in the denominator, so an Android project that
   * fixed every single gap launchpad could find still topped out at **73%**.
   * Four rows said "only you can confirm this" and then offered no way to
   * confirm anything, two of them marked critical. The reward for doing all the
   * work was a failing grade, which is the opposite of what a product for
   * people who feel stuck should do. The denominator is now the decidable
   * checks only, so finishing the work reaches 100.
   *
   * Second, a percentage is precise where this tool is uncertain and
   * demoralising where it should teach. "9 things between this and a product"
   * carries the same information, sorts itself into an order, and reads as a
   * list to work through rather than a mark out of ten.
   */
  score: number;
  applicable: number;
  passed: number;
  /** Gaps launchpad can see. THE number: what is between this app and a product. */
  remaining: number;
  /**
   * Things only the user can answer. Not failures, and never counted as such —
   * a keystore backup lives in a drawer, not in a repository.
   */
  unconfirmed: number;
  /** Where to start: the highest-consequence gap. A queue needs a front. */
  next?: Check;
}

const has = (repo: string, rel: string) => existsSync(join(repo, rel));
const read = (repo: string, rel: string): string => {
  try { return readFileSync(join(repo, rel), 'utf8'); } catch { return ''; }
};
const ls = (repo: string, rel: string): string[] => {
  try { return readdirSync(join(repo, rel)); } catch { return []; }
};

const archetypes = (st: LaunchpadState) => new Set(st.surfaces.map(s => s.archetype));
 ...
 */
const DELIVERS_TO: Record<Archetype, string> = {
...
function deliveryPhrase(kinds: Set<Archetype>): string {
  const order: Archetype[] = ['ios', 'android', 'macos-dmg', 'web-app', 'static-site'];
  const parts = order.filter(a => kinds.has(a)).map(a => DELIVERS_TO[a]);
  if (parts.length <= 1) return parts[0] ?? 'anywhere a stranger could reach it';
 ...
 */
function findDoc(repo: string, pattern: RegExp, maxDepth = 6): boolean {
  const walk = (rel: string, depth: number): boolean => {
    ...
    for (const name of ls(repo, rel)) {
      ...
      if (ls(repo, child).length && walk(child, depth + 1)) return true;
    }
    return false;
  };
  return walk('', 0);
 ...
 */
function grepRepo(repo: string, fileName: RegExp, content: RegExp, maxDepth = 5, hidden = false): boolean {
  const walk = (rel: string, depth: number): boolean => {
    if (depth > maxDepth) return false;
    for (const name of ls(repo, rel)) {
      ...
      if (isDir) {
        ...
        if (name.startsWith('.') && !name.endsWith('.xcodeproj') && !hidden) continue;
...
function grade(ok: boolean, detail: string, critical = false): Pick<Check, 'grade' | 'detail' | 'critical'> {
 ...
 */
function graded(
  evidence: 'ok' | 'unknown' | 'gap', detail: string, unknownDetail: string, critical = false,
): Pick<Check, 'grade' | 'detail' | 'critical'> {
  return {
    grade: evidence,
...
function ciFiles(repo: string): Array<{ name: string; text: string }> {
  const out: Array<{ name: string; text: string }> = [];
  for (const name of ls(repo, '.github/workflows')) {
    if (/\.ya?ml$/.test(name)) out.push({ name, text: read(repo, join('.github/workflows', name)) });
  }
...
function ignoredByGit(gitignore: string, relPath: string): boolean {
 ...
 */
function envFiles(repo: string, maxDepth = 3): string[] {
  const found: string[] = [];
  const walk = (rel: string, depth: number): void => {
    ...
    for (const name of ls(repo, rel)) {
      if (SKIP_DIRS.has(name)) continue;
      const child = rel ? join(rel, name) : name;
      if (ls(repo, child).length) { if (!name.startsWith('.')) walk(child, depth + 1); continue; }
...
export function scorecard(repo: string, st: LaunchpadState): Scorecard {
  ...
  add({
    ...
    ...grade(st.surfaces.some(s => (s.config as { tagPrefix?: string } | undefined)?.tagPrefix) || has(repo, 'CHANGELOG.md'),
      'No release process is configured, so versions get bumped by hand and eventually wrong. ' +
  ...
  if (kinds.has('android')) {
    ...
    add({
      ...
      grade: 'unknown',
      detail:
        'Only you can confirm this. If the Android upload keystore is lost, that app can NEVER ' +
  ...
  add({
    ...
    ...graded(
      wired.length === 0 ? 'gap' : hasReleaseTag(repo) ? 'ok' : 'unknown',
      kinds.size
        ...
        : eco
          ...
          : 'Nothing is wired to get this in front of anyone, and launchpad cannot tell what '
            + 'shape "shipped" takes here. Whatever it is, it exists only where you are sitting.',
      `${wired.length} surface(s) are wired, but nothing in this repo shows a release has ever ` +
  ...
  if (isDesktop) {
    const mac = configOf<MacosConfig>(st, 'macos-dmg');
    ...
    add({
      ...
      ...grade(hasUpdater,
        'A directly-distributed desktop app with no updater strands every user on the version they '
  ...
  add({
    ...
    ...graded(
      gatesOnPush ? 'ok' : localGate ? 'unknown' : 'gap',
      'Nothing checks a commit before it can ship. The cheap version of this runs on Linux and ' +
      'costs roughly nothing; skipping it means CI only tells you about problems after a build.',
      'There is a local gate in this repo, but nothing in CI runs your tests on a push — so it ' +
  ...
  add({
    ...
    ...graded(
      exposed.length ? 'gap' : envs.length ? 'ok' : /(^|\n)\s*!?\.?\*?\.env/.test(gitignore) ? 'ok' : 'unknown',
      `${exposed.slice(0, 3).map(f => `\`${f}\``).join(', ')}${exposed.length > 3 ? ` and ${exposed.length - 3} more` : ''} ` +
      ...
      'not remove it from the history; it has to be rotated.',
      'No .env file here and nothing in .gitignore that would cover one. Nothing is wrong today, ' +
      'but the first time you add one it gets committed by default.',
      true),
  });
  ...
  add({
    ...
    ...grade(/sentry|crashlytics|bugsnag|firebase_crashlytics/i.test(deps),
      'Without crash reporting you learn about crashes from a one-star review, if at all.'),
  });
  add({
    id: 'analytics', title: 'Install / usage visibility',
    ...grade(/posthog|plausible|amplitude|mixpanel|firebase_analytics|umami|fathom/i.test(deps),
      'You cannot tell whether anyone is using this. One number per app is enough; it does not ' +
      'need to be a BI stack.'),
  });

  // ── 12. money, and doing it the way the platform demands ──────────────────
  const macPay = configOf<MacosConfig & { payments?: unknown }>(st, 'macos-dmg');
  ...
  add({
    ...
    ...(kinds.has('static-site') || kinds.has('web-app') || Boolean(configOf<SiteConfig>(st, 'static-site'))
      ? grade(true, '')
      : servesItsOwnPage
        ...
        : eco && generic && (eco.ships === 'cli' || eco.ships === 'library')
          ...
          ? { grade: 'n/a' as Grade, detail: '' }
        : eco && (eco.ships === 'cli' || eco.ships === 'library')
          ? grade(readme.length > 400,
            `For ${article(eco.name)} ${eco.name} the page IS the README — it is what ${eco.registry ?? 'a registry'} `
          ...
          : grade(false,
            'There is no page describing this app. Every channel you might use later — a store '
  ...
  if (eco && eco.ships !== 'app' && generic) {
    add({
      ...
      ...grade(/^#/m.test(readme) && readme.length > 400,
        readme
    ...
    add({
      ...
      ...grade(
        Boolean(ls(repo, '').find(n => /^(LICENSE|LICENCE|COPYING)(\.(md|txt))?$/i.test(n)))
        || LICENCE_FIELD.test(deps + read(repo, 'Cargo.toml') + read(repo, 'go.mod') + read(repo, 'pyproject.toml')),
        eco.ships === 'library' || eco.ships === 'cli'
    ...
    if (eco.ships === 'service') {
      ...
      add({
        ...
        ...graded(deployable ? 'unknown' : 'gap',
          'Nothing here describes how this gets deployed — no container, no platform config, no '
          + 'deploy step in CI. Until one exists, "shipping" means you, by hand, from this machine, '
          + 'and only while it is switched on.',
          'There is a deploy configuration, but a repo cannot tell whether it is actually running '
      ...
      add({
        ...
        ...(eco.registry
          ? graded(
            INSTALL_CMD.test(readme) ? 'ok' : hasReleaseTag(repo) ? 'unknown' : 'gap',
            `Nothing here says how to get a copy. For ${article(eco.name)} ${eco.name} the finish line is `
            + `${installPhrase(eco)} — until then it is a repository, not something anyone can use.`,
            'There are release tags but no install line in the README. Whether this is actually on '
            + `${eco.registry} is something only you can confirm.`)
          : graded(hasReleaseTag(repo) ? 'unknown' : 'gap',
            'Nothing here produces something a stranger can download — no release, no packaged '
            + 'artifact. A build on your machine is not a copy anyone else can run.',
            'There are release tags, but whether they carry a runnable artifact for anyone else is '
  ...
  if (iconReq || kinds.has('web-app') || kinds.has('static-site')) {
    ...
    add(!iconReq
      ...
      ? {
        ...
        ...(!icon
          ? grade(false,
            'No icon or favicon found. Every browser tab, bookmark and shared link shows one, and its '
      ...
      : {
        ...
        ...(!icon
          ? grade(false,
            `No app icon found. ${iconReq.store} requires a ${iconReq.size}×${iconReq.size} icon before it `
            + 'will accept a build at all, so this blocks submission rather than looking unfinished.')
          : alphaProblem
            ? grade(false,
              `\`${icon.path}\` has an alpha channel. ${iconReq.store} rejects icons with transparency — `
            ...
            : !bigEnough
              ? grade(false,
                `\`${icon.path}\` is ${icon.width}×${icon.height}. ${iconReq.store} requires `
  ...
  if (hasStoreSurface(st.surfaces)) {
    ...
    add({
      ...
      ...grade(
        Boolean(assets.listingDir) && assets.screenshots.length > 0 && (!needsFeature || Boolean(fg)),
        [
          'A store listing is its own deliverable, and it is the step most people have not started.',
          assets.screenshots.length ? null : (
            kinds.has('ios') && kinds.has('android')
...
function rank(c: Check): number {
  if (c.grade === 'gap') return c.critical ? 0 : 1;
  if (c.grade === 'unknown') return c.critical ? 2 : 3;
  return 4;
}
...
export function renderScorecard(s: Scorecard): string {
  const icon: Record<Grade, string> = { ok: '✓', gap: '✗', unknown: '?', 'n/a': '·' };
  const out: string[] = [];
