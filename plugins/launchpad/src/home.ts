 ...
 */
export function launchpadHome(env: NodeJS.ProcessEnv = process.env, cwd = process.cwd()): string {
  const explicit = env.LAUNCHPAD_HOME?.trim();
  if (explicit) return explicit;
  const local = readLocalOverride(cwd);
  if (local.home) return local.home;
  const fromEnv = (process.platform === 'win32' ? env.USERPROFILE : env.HOME)?.trim();
  if (fromEnv) return fromEnv;
  return homedir();
}

/** What `.launchpad/local.json` is allowed to say. Anything else is ignored. */
export interface LocalOverride {
  /** Resolved to an absolute path against the repository root. */
  home?: string;
  /** Validated by `vault.ts`, not here — this file owns no vault vocabulary. */
  vaultBackend?: string;
  /** Where it was read from. Useful in a diagnostic; never rendered to a buyer. */
  from?: string;
}
 ...
 * this is for.
 */
export function localOverridePath(cwd = process.cwd()): string | null {
  let dir = resolve(cwd);
 ...
 */
export function readLocalOverride(cwd = process.cwd()): LocalOverride {
 ...
 * unpacked.
 */
function resolveHome(value: string, root: string): string {
 ...
 */
export function applyHomeOverride(env: NodeJS.ProcessEnv = process.env, cwd = process.cwd()): string {
