 ...
 */
export const demoRoot = (pid = process.pid): string => join(tmpdir(), `launchpad-demo-${pid}`);

/** The HOME the dashboard is pointed at. Holds the registry and nothing else. */
export const demoHome = (root = demoRoot()): string => join(root, 'home');

/** Where the fictional repos live. `Code/<name>` so the header reads like a real path. */
export const demoCode = (root = demoRoot()): string => join(root, 'Code');

const w = (file: string, text: string): void => {
  mkdirSync(dirname(file), { recursive: true });
  writeFileSync(file, text, 'utf8');
};
const wb = (file: string, buf: Buffer): void => {
  mkdirSync(dirname(file), { recursive: true });
 ...
 */
function gitRepo(repo: string, opts: { branch?: string; tags?: Record<string, string>; head?: string }): void {
...
function dateTag(repo: string, tag: string, iso: string): void {
  const t = new Date(iso);
  try { utimesSync(join(repo, '.git', 'refs', 'tags', tag), t, t); } catch { /* best effort */ }
}

const VALIDATE = (test: string) => `name: launchpad · validate
on:
  push:
    branches: ['**']
  pull_request:
...
function base(repo: string, o: {
  ...
  w(join(repo, '.gitignore'),
    ['node_modules/', 'build/', 'dist/', ...(o.gitignoreEnv === false ? [] : ['.env', '.env.*']), ''].join('\n'));
  ...
  if (o.deps) w(join(repo, 'package.json'), o.deps);
}

const pkg = (name: string, deps: Record<string, string>): string =>
  JSON.stringify({ name, private: true, version: '1.0.0', scripts: { test: 'vitest run' }, dependencies: deps }, null, 2) + '\n';

/** iOS/Android/macOS listing assets, in the places a store tool actually keeps them. */
function storeAssets(repo: string, name: string, o: { platform: string; feature?: boolean; shots?: number }): void {
  ...
  for (let i = 1; i <= (o.shots ?? 3); i++) {
    wb(join(repo, 'fastlane', 'screenshots', o.platform, `${String(i).padStart(2, '0')}-home.png`),
      screenshot(name, i));
  }
  if (o.feature) wb(join(meta, 'images', 'featureGraphic.png'), featureGraphic(name));
}

/** The one image that makes a list of projects scannable. 1024, opaque — Apple's rule. */
const icon = (repo: string, name: string, rel = join('assets', 'icon.png')): void =>
  wb(join(repo, rel), appIcon(name, 1024));

// ─────────────────────────────────────────────────────────────────────────────
// The fleet. Seven invented apps across every surface launchpad ships, in the
// states a real fleet is in: two finished, three within touching distance, one
// just started. Nothing here is anybody's real project, and nothing in it is
// tenant-specific.
// ─────────────────────────────────────────────────────────────────────────────
...
export interface DemoApp {
  ...
  /** Written into state.yml. The archetypes, statuses and gathered config. */
  state: (repo: string) => LaunchpadState;
}

const S = (
  id: string, archetype: LaunchpadState['surfaces'][number]['archetype'],
  evidence: string[], config: Record<string, unknown>,
): LaunchpadState['surfaces'][number] =>
  ({ id, archetype, status: 'wired', confidence: 'high', evidence, config: config as never });
...
export const DEMO_APPS: DemoApp[] = [
  {
    ...
    build: repo => {
      ...
      w(join(repo, '.launchpad', 'macos', 'create-dmg.sh'),
        ['#!/usr/bin/env bash', 'set -euo pipefail', 'codesign --deep --options runtime --timestamp "$APP"',
          'xcrun notarytool submit "$DMG" --keychain-profile launchpad --wait',
          'xcrun stapler staple "$DMG"', ''].join('\n'));
      w(join(repo, '.github', 'workflows', 'launchpad-macos-release.yml'),
        ['name: launchpad · macOS release', 'on:', '  push:', "    tags: ['v*']", 'jobs:', '  release:',
...

  {
    ...
    build: repo => {
      ...
      w(join(repo, 'apps', 'mobile', 'pubspec.yaml'),
        ['name: fernweh', 'environment:', "  sdk: '>=3.4.0 <4.0.0'", 'dependencies:', '  flutter:',
      ...
      w(join(repo, 'apps', 'mobile', 'android', 'app', 'build.gradle.kts'),
        ['android {', '    signingConfigs {', '        create("release") {',
          '            storeFile = file(System.getenv("ANDROID_KEYSTORE_PATH") ?: "upload.jks")',
          '        }', '    }', '    buildTypes {', '        release {',
          '            signingConfig = signingConfigs.getByName("release")', '        }', '    }', '}', ''].join('\n'));
      w(join(repo, '.github', 'workflows', 'launchpad-mobile-release.yml'),
        ['name: launchpad · mobile release', 'on:', '  push:', "    tags: ['v*']", '  workflow_dispatch:',
...

  {
    ...
    build: repo => {
      ...
      w(join(repo, 'Package.swift'),
        ['// swift-tools-version:5.10', 'import PackageDescription', 'let package = Package(',
          '  name: "Ketchbook",', '  dependencies: [',
          '    .package(url: "https://github.com/getsentry/sentry-cocoa", from: "8.0.0"),',
          '    .package(url: "https://github.com/PostHog/posthog-ios", from: "3.0.0"),', '  ])', ''].join('\n'));
      w(join(repo, '.github', 'workflows', 'launchpad-ios-release.yml'),
        ['name: launchpad · iOS release', 'on:', '  push:', "    tags: ['v*']", '  workflow_dispatch:',
...

  {
    ...
    build: repo => {
      ...
      w(join(repo, 'android', 'app', 'build.gradle.kts'),
        ['android {', '    signingConfigs {', '        create("release") {',
          '            storeFile = file(System.getenv("ANDROID_KEYSTORE_PATH") ?: "upload.jks")',
          '        }', '    }', '    buildTypes { release { signingConfig = signingConfigs.getByName("release") } }',
          '}', ''].join('\n'));
      w(join(repo, '.github', 'workflows', 'launchpad-android-release.yml'),
        ['name: launchpad · Android release', 'on:', '  schedule:', "    - cron: '0 3 * * *'",
...

  {
    ...
    build: repo => {
      ...
      w(join(repo, 'LICENSE'), 'MIT License\n\nCopyright (c) 2026\n');
      w(join(repo, 'Cargo.toml'),
        ['[package]', 'name = "whetstone"', 'version = "0.1.0"', 'edition = "2021"', 'license = "MIT"',
 ...
 */
function nightshift(repos: Record<string, string>): void {
  ...
  w(join(fernweh, '.launchpad', 'nightshift', 'inbox.md'),
    ['- A widget that shows the next leg of the trip', '- Dark map tiles', ''].join('\n'));
 ...
 */
export function materializeDemo(root = demoRoot()): string {
