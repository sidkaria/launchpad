...
export function png(width: number, height: number, at: (x: number, y: number) => RGB): Buffer {
 ...
 */
export const hueOf = (name: string): number => hashOf(name) % 360;
...
const GLYPHS: Array<(u: number, v: number) => boolean> = [
  // ascending bars
 ...
 * apart.
 */
export function appIcon(name: string, size = 1024): Buffer {
  const h = hueOf(name);
...
export function featureGraphic(name: string, w = 1024, h = 500): Buffer {
