 ...
 */
export const WATCH_IGNORED: ReadonlySet<string> = new Set([
...
const segments = (rel: string): string[] => rel.split(/[\\/]/).filter(Boolean);
 ...
 */
export function watchWorthy(rel: string | null | undefined): boolean {
...
export function gitWorthy(rel: string | null | undefined): boolean {
  if (rel == null || rel === '') return true;
...
export function watchableDirs(repo: string): string[] {
  let entries;
  try { entries = readdirSync(repo, { withFileTypes: true }); } catch { return []; }
  return entries
...
export interface FleetWatch {
  /**
   * Subscribe to anything not yet subscribed to. Idempotent and cheap — one
   * `readdir` per project — so it is safe to call on every coalesced burst,
   * which is what picks up a project registered while the page is open and a
   * `.github/` created after the connection opened.
   */
  arm: () => void;
  /** Every directory currently subscribed to. */
  dirs: () => string[];
  /** Directories the platform refused to watch. */
  refused: () => string[];
  /** True once the watcher ceiling has been reached. */
  capped: () => boolean;
  /**
   * What to tell the page about its own liveness, or null when everything is
   * watched. Never a claim that updates are live when they are not: a dashboard
   * that has silently stopped updating is worse than one that says it is slow.
   */
  notice: () => { live: boolean; watching: number; degraded: boolean; reason: string | null };
  close: () => void;
}
...
export function watchFleet(opts: {
  home: string;
  projects: () => string[];
  /** An adopted backlog can live outside `.launchpad`. */
  backlogDirOf?: (repo: string) => string | undefined;
  onChange: () => void;
  limit?: number;
}): FleetWatch {
  ...
  const subscribe = (dir: string, recursive: boolean, worthy: (rel: string | null) => boolean): void => {
    if (watchers.has(dir)) return;
    if (watchers.size >= limit) { capped = true; return; }
    if (!existsSync(dir)) return;
    try {
      const w = watch(dir, { recursive }, (_event, name) => {
        if (worthy(name === null ? null : String(name))) opts.onChange();
      });
      ...
      w.on('error', () => { try { w.close(); } catch { /* already gone */ } watchers.delete(dir); });
      watchers.set(dir, w);
    } catch {
      ...
      if (!refused.includes(dir)) refused.push(dir);
    }
  };
  ...
  const arm = (): void => {
    ...
    for (const repo of opts.projects()) {
      ...
      for (const dir of watchableDirs(repo)) subscribe(dir, true, watchWorthy);
      ...
      if (backlog) subscribe(join(repo, backlog), true, watchWorthy);
    }
  };
