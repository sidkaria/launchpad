 ...
 */
export function refusedWhy(error: string | null | undefined): string {
  if (!error) return 'Lemon Squeezy rejected that key.';
...
export const lemonSqueezy: LicenseProvider = {
  ...
  async activate(key, instanceName, fetcher): Promise<Activation> {
    ...
    try {
      ...
      body = (await res.json()) as LsResponse;
      if (!body || typeof body !== 'object') return { status: 'unreachable', why: 'unrecognised response' };
    } catch (e) {
      return { status: 'unreachable', why: (e as Error).message };
    }
    if (!body?.activated) {
