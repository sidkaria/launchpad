 ...
 */
export function pluginCatalog(dir: string): Map<string, string> {
  const out = new Map<string, string>();
  const gradleDir = join(dir, 'gradle');
  for (const name of ls(gradleDir)) {
    if (!name.endsWith('.versions.toml')) continue;
 ...
 */
export function parsePluginsTable(toml: string): Map<string, string> {
  const out = new Map<string, string>();
  let inPlugins = false;
  for (const raw of toml.split('\n')) {
    ...
    if (section) { inPlugins = section[1] === 'plugins'; continue; }
 ...
 */
export function appliesPlugin(buildText: string, pluginId: string, catalog: Map<string, string>): boolean {
  ...
  for (const raw of buildText.split('\n')) {
    ...
    if (/\bapply\s+false\b/.test(raw)) continue;
    const line = raw.replace(/\/\/.*$/, '');
    if (idPattern.test(line)) return true;
    ...
    for (const m of line.matchAll(/\balias\s*\(\s*([A-Za-z0-9_.]+)\s*\)|\balias\s+([A-Za-z0-9_.]+)/g)) {
      if (catalog.get(m[1] ?? m[2]) === pluginId) return true;
    }
  }
  return false;
}
...
export const moduleBuildText = (dir: string, rel: string): string =>
  read(join(dir, rel, 'build.gradle')) + '\n' + read(join(dir, rel, 'build.gradle.kts'));

/** Candidate module directories: the root itself, plus its immediate children. */
export function moduleDirs(dir: string): string[] {
 ...
 */
export function androidAppModule(dir: string): string | null {
  const catalog = pluginCatalog(dir);
  for (const rel of moduleDirs(dir)) {
    if (appliesPlugin(moduleBuildText(dir, rel), ANDROID_APPLICATION_PLUGIN, catalog)) return rel;
  }
  return null;
}

/** Does any module apply `com.android.library`? Used only to explain a no-app answer. */
export function hasAndroidLibraryModule(dir: string): boolean {
  const catalog = pluginCatalog(dir);
  return moduleDirs(dir).some(rel =>
    appliesPlugin(moduleBuildText(dir, rel), ANDROID_LIBRARY_PLUGIN, catalog));
}

/** A bounded grep for a literal string under one directory. */
function containsUnder(dir: string, rel: string, needle: string, maxDepth = 4): string | null {
  const walk = (cur: string, depth: number): string | null => {
    if (depth > maxDepth) return null;
    for (const name of ls(join(dir, cur))) {
      if (name === 'build' || name === '.gradle' || name.startsWith('.')) continue;
 ...
 */
export function weakAndroidEvidence(dir: string): string | null {
 ...
 */
export function launcherManifest(dir: string, maxDepth = 4): string | null {
  const walk = (cur: string, depth: number): string | null => {
    if (depth > maxDepth) return null;
    for (const name of ls(join(dir, cur))) {
      if (name === 'build' || name === '.gradle' || name.startsWith('.')) continue;
