...
export function bumpXcodeVersion(pbxproj: string, version: string): string {
  return pbxproj
    .replace(/MARKETING_VERSION = [^;]*;/g, `MARKETING_VERSION = ${version};`)
    .replace(/CURRENT_PROJECT_VERSION = [^;]*;/g, `CURRENT_PROJECT_VERSION = ${version};`);
}

export function bumpXcodegenVersion(yaml: string, version: string): string {
  return yaml
    .replace(/MARKETING_VERSION:.*/g, `MARKETING_VERSION: "${version}"`)
    .replace(/CURRENT_PROJECT_VERSION:.*/g, `CURRENT_PROJECT_VERSION: "${version}"`);
}
 ...
 * whole build.
 */
export function bumpPubspecVersion(yaml: string, version: string): string {
  if (!PUBSPEC_VERSION.test(yaml)) {
    throw new Error(
      'pubspec.yaml has no top-level `version:` line — add one (e.g. `version: 1.0.0+1`) before releasing.',
    );
  }
  return yaml.replace(PUBSPEC_VERSION, (_full, _marketing, build: string | undefined, tail: string) =>
    `version: ${version}+${Number(build ?? 0) + 1}${tail}`);
}
