...
export interface SiteConfig {
  appName: string;
  siteDir: string;       // repo-relative dir of the static site (e.g. 'site' or '.')
  productionBranch: string;   // branch that deploys to production (default 'main'); others → preview
...
const underDir = (dir: string, rel: string) => (repoRelDir(dir) ? `${repoRelDir(dir)}/${rel}` : rel);
 ...
 */
export function sitePathsFilter(siteDir: string): string {
