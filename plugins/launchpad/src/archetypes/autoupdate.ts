...
export function planAutoupdateFiles(opts: { appSourceDir: string }): GeneratedFile[] {
 ...
 */
export function writeAutoupdateFiles(repo: string, opts: { appSourceDir: string }): WriteResult {
  return writeGuarded(repo, planAutoupdateFiles(opts));
}

export function injectSparklePackage(projectYml: string, macosTarget: string): string {
  ...
  if (target) {
    ...
    const present = target.dependencies.some((d: any) => d && d.package === 'Sparkle');
    if (!present) target.dependencies.push({ package: 'Sparkle', product: 'Sparkle' });
  }
...
export function wireAutoupdate(
  repo: string,
  c: { scheme: string; appcastDomain: string; appSourceDir?: string; publicEdKey?: string },
): string[] {
  if (!c.appSourceDir || !c.publicEdKey) return [];
  ...
  patch(`${appSourceDir}/Info.plist`,
    (s) => injectAutoupdateInfoPlistKeys(s, { feedUrl, publicEdKey }));

  return changed;
}

export function injectAutoupdateInfoPlistKeys(
  plist: string,
  opts: { feedUrl: string; publicEdKey: string },
): string {
  ...
  for (const [key, valueEl] of desired) {
    ...
    const re = new RegExp(
      `(<key>${key}</key>\\s*)(<string>[^<]*</string>|<true\\s*/>|<false\\s*/>|<integer>[^<]*</integer>)`,
    );
    if (re.test(out)) {
