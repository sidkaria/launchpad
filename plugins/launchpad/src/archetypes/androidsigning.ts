 ...
 */
export function injectAndroidReleaseSigning(kts: string): string {
  if (hasReleaseSigningConfig(kts)) return kts;
  ...
  if (rel) {
    ...
    if (ANY_SIGNING.test(inner) && !DEBUG_SIGNING.test(inner)) return kts;
 ...
 */
export function wireAndroidReleaseSigning(repo: string, c: { workdir: string }): string[] {
  const rel = c.workdir === '.'
...
function hasReleaseSigningConfig(kts: string): boolean {
  const re = new RegExp(SIGNING_CONFIGS_BLOCK.source, 'g');
  for (let m = re.exec(kts); m; m = re.exec(kts)) {
    const open = kts.indexOf('{', m.index);
    if (/create\s*\(\s*"release"\s*\)/.test(kts.slice(open, matchBrace(kts, open, ktSkip)))) return true;
  }
  return false;
}
...
function findKtBlock(src: string, re: RegExp, from: number, until: number): KtBlock | undefined {
...
function patchReleaseBody(inner: string, indent: string): string {
...
function addReleaseBuildType(inner: string, indent: string): string {
  const i1 = `${indent}    `;
...
function addPropertiesImport(kts: string): string {
  if (HAS_PROPERTIES_IMPORT.test(kts)) return kts;
...
function firstStatementStart(kts: string): number {
  let at = 0;
  let inComment = false;
 ...
 */
function signingConfigsBlock(i1: string): string {
...
function buildTypesBlock(i1: string): string {
  return [
    `${i1}buildTypes {`,
    `${i1.repeat(2)}release {`,
