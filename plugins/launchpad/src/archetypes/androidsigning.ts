...
import { androidFramework, type AndroidConfig } from './android.js';
 ...
 * so. A native Android or React Native app ships with no release signing at
 * all, which is the same trap wearing a different coat: `assembleRelease`
 * succeeds and hands you `app-release-unsigned.apk`. launchpad's Android
 * workflows already materialise the real keystore and write `key.properties`
 * in CI; this module closes the last gap by rewriting the Gradle file to USE it.
 *
 * Two things it has to get right that the Flutter-only version did not:
 *
 * **Both DSLs.** A Flutter app's `android/app/build.gradle.kts` is Kotlin, so
 * the original transform only ever spoke Kotlin and skipped anything else
 * rather than corrupting it. Native Android and React Native are both still
 * commonly Groovy — React Native's own template is — so skipping there would
 * mean the majority of real apps get a workflow that builds an unsigned APK
 * and no explanation.
 *
 * **A missing keystore must not break the build.** The obvious assignment,
 * `signingConfig = signingConfigs.getByName("release")`, makes a release build
 * with no `key.properties` fail at `validateSigningRelease` with "Keystore file
 * not set for signing config release" — so a contributor who clones the repo
 * and runs `./gradlew assembleRelease`, or a CI run before the secret is
 * stored, gets a Gradle error about a file they have never heard of. The
 * assignment is therefore conditional on the store file actually being there:
 * signed when it is, plainly unsigned when it is not, and never falling back to
 * the debug keystore, which is the failure this module exists to remove.
 *
...
export type GradleDsl = 'kotlin' | 'groovy';
 ...
 */
const releaseSigningAssignment = (dsl: GradleDsl): string =>
  dsl === 'kotlin'
 ...
 */
export function groovySkip(src: string, i: number): number {
  if (src[i] === "'") {
...
const skipperFor = (dsl: GradleDsl) => (dsl === 'groovy' ? groovySkip : ktSkip);

/** Which DSL a build file is written in, from its name. */
export const dslOf = (relPath: string): GradleDsl =>
  (relPath.endsWith('.kts') ? 'kotlin' : 'groovy');
 ...
 */
export function injectAndroidReleaseSigning(text: string, dsl: GradleDsl = 'kotlin'): string {
  if (hasReleaseSigningConfig(text, dsl)) return text;
  const skip = skipperFor(dsl);

  const androidM = /(^|\n)([ \t]*)android\s*\{/.exec(text);
  if (!androidM) return text;
  ...
  if (rel) {
    ...
    if (ANY_SIGNING.test(inner) && !DEBUG_SIGNING.test(inner)) return text;
 ...
 */
export function injectAndroidVersionCode(text: string, dsl: GradleDsl = 'kotlin'): string {
  if (text.includes(VERSION_CODE_PROPERTY)) return text;
  const skip = skipperFor(dsl);
  const androidM = /(^|\n)([ \t]*)android\s*\{/.exec(text);
  if (!androidM) return text;
  const androidOpen = text.indexOf('{', androidM.index + androidM[1].length);
 ...
 */
export function appBuildFile(repo: string, c: AndroidConfig): string | null {
 ...
 */
export function ownReleaseSigning(
  repo: string, c: AndroidConfig,
): { path: string; verdict: ReleaseSigningVerdict } | null {
  const rel = appBuildFile(repo, c);
  if (!rel) return null;
  const text = readFileSync(join(repo, rel), 'utf8');
  const dsl = dslOf(rel);
  if (!hasReleaseSigningConfig(text, dsl)) return null;
  // Ours, from a previous `apply` — that one reads key.properties by
  // construction, so there is nothing to warn about.
  if (text.includes('key.properties')) return null;
 ...
 */
export function wireAndroidReleaseSigning(repo: string, c: AndroidConfig): string[] {
  const rel = appBuildFile(repo, c);
  if (!rel) return [];
  ...
  if (isGradleFramework(androidFramework(c))) next = injectAndroidVersionCode(next, dsl);
...
function hasReleaseSigningConfig(text: string, dsl: GradleDsl): boolean {
  const skip = skipperFor(dsl);
  ...
  for (let m = re.exec(text); m; m = re.exec(text)) {
    ...
    if (declares.test(text.slice(open, matchBrace(text, open, skip)))) return true;
  }
  return false;
}

interface GradleBlock {
  lineStart: number;   // first char of the header's own line
  indent: string;      // that line's leading whitespace
  open: number;        // index of the opening `{`
  close: number;       // index of the matching `}`
}

type Skipper = (src: string, i: number) => number;

/** First block matching `re` (which must start `\n([ \t]*)`) strictly inside (from, until). */
function findBlock(src: string, re: RegExp, from: number, until: number, skip: Skipper): GradleBlock | undefined {
  const r = new RegExp(re.source, 'g');
  r.lastIndex = from;
  const m = r.exec(src);
  if (!m || m.index >= until) return undefined;
...
function addReleaseBuildType(inner: string, indent: string, dsl: GradleDsl): string {
  const i1 = `${indent}    `;
...
function addPropertiesImport(text: string, dsl: GradleDsl): string {
  if (dsl !== 'kotlin') return text;
  if (HAS_PROPERTIES_IMPORT.test(text)) return text;
...
function firstStatementStart(text: string): number {
  let at = 0;
  let inComment = false;
  for (const line of text.split('\n')) {
    const t = line.trim();
    if (inComment) inComment = !t.includes('*/');
    else if (t.startsWith('/*')) inComment = !t.includes('*/');
    else if (t !== '' && !t.startsWith('//')) return at;
    at += line.length + 1;
  }
  return 0;   // nothing but comments — unreachable once an `android { }` block was found
}

function firstIndent(lines: string[]): string | undefined {
 ...
 */
function signingConfigsBlock(i1: string, dsl: GradleDsl): string {
...
function buildTypesBlock(i1: string, dsl: GradleDsl): string {
  return [
    `${i1}buildTypes {`,
    `${i1.repeat(2)}release {`,
...
/** What an app module's build file says about signing its RELEASE build. */
export type ReleaseSigningVerdict =
  /** Nothing signs it, or it is signed with the DEBUG keystore. */
  | 'none'
  /** A named config with a store file it can actually resolve. */
  | 'signed'
  /**
   * A named config exists, and nothing in the repository gives it a keystore.
   * Something outside the build file is expected to (the app's own lanes, an
   * environment variable) — which may be perfectly fine and is not observable
   * from here.
   */
  | 'declared';
 ...
 */
export function releaseSigning(text: string, dsl: GradleDsl = 'kotlin'): ReleaseSigningVerdict {
  const skip = skipperFor(dsl);
  const androidM = /(^|\n)([ \t]*)android\s*\{/.exec(text);
  if (!androidM) return 'none';
  const androidOpen = text.indexOf('{', androidM.index + androidM[1].length);
  const androidClose = matchBrace(text, androidOpen, skip);
  ...
  if (!rel) return 'none';
  const inner = text.slice(rel.open + 1, rel.close);
  if (!ANY_SIGNING.test(inner) && !/\bsigningConfig\b/.test(inner)) return 'none';
  if (DEBUG_SIGNING.test(inner)) return 'none';
