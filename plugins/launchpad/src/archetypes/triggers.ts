...
export type DistributeTrigger = 'push' | 'tag' | 'dispatch' | 'schedule';

/** Defaults preserving each surface's pre-policy behaviour. */
export const IOS_DEFAULT_DISTRIBUTE_ON: DistributeTrigger[] = ['push', 'tag', 'dispatch'];
export const ANDROID_DEFAULT_DISTRIBUTE_ON: DistributeTrigger[] = ['push', 'dispatch'];
export const IOS_DEFAULT_RUNNER = 'macos-15';
export const ANDROID_DEFAULT_RUNNER = 'ubuntu-latest';

/** The subset of a mobile config that decides when (and where) CI runs. */
export interface TriggerPolicy {
  testBranch: string;
  tagPrefix?: string;
  distributeOn: DistributeTrigger[];
  schedule?: string;      // cron (UTC), required when distributeOn includes 'schedule'
  pathFilter?: string[];  // repo-relative globs
}
 ...
 */
export function repoRelDir(dir: string): string {
  return (dir ?? '').trim()
 ...
 */
export function pathGlob(dir: string): string {
 ...
 */
export function renderOnBlock(p: TriggerPolicy): string {
  ...
  if (on.has('dispatch')) lines.push('  workflow_dispatch:');
  ...
  if (on.has('push') || on.has('tag')) {
    lines.push('  push:');
    if (on.has('push')) lines.push('    branches:', `      - '${p.testBranch}'`);
    if (on.has('tag')) lines.push('    tags:', `      - '${p.tagPrefix ?? 'v'}*'`);
 ...
 */
export function guardPaths(p: { pathFilter?: string[] }, workdir: string): string[] {
 ...
 */
export function stepIfLine(distributeOn: DistributeTrigger[]): string {
 ...
 */
export function stepIfVar(distributeOn: DistributeTrigger[]): string {
 ...
 */
export function scheduleGuardStep(p: TriggerPolicy, workdir: string): string {
  if (!p.distributeOn.includes('schedule')) return '';
 ...
 */
export function flutterSetupStep(version: string | undefined, stepIf: string): string {
