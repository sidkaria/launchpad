...
export const CHECKOUT_URL =
  process.env.LAUNCHPAD_CHECKOUT_URL ?? 'https://launchpad.lemonsqueezy.com/checkout';
...
export const UPDATE_POLICY =
  'all 1.x updates included; 2.0 is a paid upgrade, discounted for owners';

/** Where someone goes when the product is wrong. */
export const SUPPORT_URL =
  process.env.LAUNCHPAD_SUPPORT_URL ?? 'https://github.com/sidkaria/launchpad/issues';
 ...
 */
export const LICENSE_PROVIDER =
  process.env.LAUNCHPAD_LICENSE_PROVIDER ?? 'lemonsqueezy';

/** Lemon Squeezy's public licence API. Overridable so CI can point at a stub. */
export const LEMONSQUEEZY_API =
  process.env.LAUNCHPAD_LEMONSQUEEZY_API ?? 'https://api.lemonsqueezy.com/v1/licenses';

/** Polar's public API root. Sandbox is `https://sandbox-api.polar.sh`. */
export const POLAR_API =
  process.env.LAUNCHPAD_POLAR_API ?? 'https://api.polar.sh';
...
export const POLAR_ORGANIZATION_ID =
  process.env.LAUNCHPAD_POLAR_ORGANIZATION_ID ?? '';
