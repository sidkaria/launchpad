/**
 * What `launchpad --help` says, and what a typo gets back.
 *
 * The old answer to both was one line — `usage: launchpad <detect|setup|…>`,
 * twenty-one verbs in a row, exit 1 — for `--help` as much as for `stauts`.
 * That line is accurate and useless: it does not say what launchpad is, which
 * three commands matter on day one, or which ones cost money.
 *
 * Most buyers never type a verb at all — they talk to Claude, and the skills
 * run the CLI for them. So the help leads with the slash commands, and lists
 * the verbs underneath for the agent and for the person who went looking.
 * `test/cli.test.ts` fails if a verb exists that this list does not name.
 */
...
export function renderHelp(price: string): string {
...
export function suggest(typed: string, verbs = VERBS.map(v => v.verb)): string | null {
