...
export interface PackagePlan {
  /** Every file that would ship, repo-root-relative, sorted. */
  files: string[];
  /** Allowlisted paths that do not exist — a build error, not a warning. */
  missing: string[];
  /** Vendored dependency roots, e.g. `plugins/launchpad/node_modules/yaml`. */
  vendored: string[];
}
...
export function planPackage(repoRoot: string): PackagePlan {
  const files: string[] = [];
  const missing: string[] = [];
  for (const p of PUBLISHED_PATHS) {
    if (!existsSync(join(repoRoot, p))) { missing.push(p); continue; }
  ...
  for (const dep of VENDORED) {
    ...
    if (!existsSync(join(repoRoot, rel))) { missing.push(rel); continue; }
    vendored.push(rel);
 ...
 */
export function leaks(plan: PackagePlan): string[] {
