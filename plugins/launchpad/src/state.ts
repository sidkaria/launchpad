...
import type { LaunchpadState, Surface, DetectedPipeline, DetectionResult } from './types.js';
...
const withDefaultDisposition = (p: DetectedPipeline): DetectedPipeline =>
  (p.disposition ? p : { ...p, disposition: DEFAULT_DISPOSITION });
 ...
 */
export function readState(repo: string): LaunchpadState | null {
  return readStateOrError(repo).state;
}

/** What went wrong, when a state file exists and cannot be used. */
export interface StateRead {
  state: LaunchpadState | null;
  /**
   * Set only when the file is PRESENT and unusable. One line, no code frame,
   * and bounded — it is rendered on a dashboard and printed to a terminal.
   */
  error?: string;
}
 ...
 */
export function readStateOrError(repo: string): StateRead {
  const p = statePath(repo);
  if (!existsSync(p)) return { state: null };
  let text: string;
  try {
  ...
  const s = raw as Partial<LaunchpadState>;
  const list = <T>(v: unknown): T[] => (Array.isArray(v) ? v.filter(x => x != null) as T[] : []);
  return {
    state: {
      project: typeof s.project === 'string' ? s.project : '',
      surfaces: list<Surface>(s.surfaces).map(f => ({ ...f, evidence: list<string>(f.evidence) })),
      pipelines: list<DetectedPipeline>(s.pipelines),
      credentialsRequired: list<string>(s.credentialsRequired),
      ...(s.notes !== undefined ? { notes: s.notes } : {}),
    },
  };
}

export function writeState(repo: string, state: LaunchpadState): void {
  const p = statePath(repo);
  mkdirSync(dirname(p), { recursive: true });
  writeFileSync(p, HEADER + stringify(state), 'utf8');
}

function mergeSurface(prev: Surface, det: Surface): Surface {
  // Archetype changed for the same id → detection wins; any stale config is dropped.
  if (prev.archetype !== det.archetype) return det;
  return {
...
export function mergeState(
  ...
  for (const p of prev.surfaces) if (!seen.has(p.id)) surfaces.push(p); // prior-only surfaces

  const key = (x: DetectedPipeline) => `${x.kind}:${x.path}`;
  ...
  for (const d of detected.pipelines) {
    ...
    if (p?.disposition) pipelines.push({ ...d, disposition: p.disposition });
    else if (p) pipelines.push(d);
    else pipelines.push(withDefaultDisposition(d));
  }
