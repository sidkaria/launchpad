...
function localBranches(repo: string): string[] {
  const out: string[] = [];
  const walk = (rel: string, prefix: string): void => {
    let names: string[];
    try { names = readdirSync(join(repo, rel)); } catch { return; }
 ...
 * under none of them.
 */
export function latestTag(repo: string): { name: string; at?: string } | null {
  const dir = join(repo, '.git', 'refs', 'tags');
  const found: Array<{ name: string; ms: number }> = [];
  const walk = (rel: string, prefix: string): void => {
    let names: string[];
    ...
    for (const name of names) {
      ...
      try {
        ...
        try { found.push({ name: `${prefix}${name}`, ms: statSync(join(dir, child)).mtimeMs }); } catch { /* vanished */ }
      }
    }
  };
  walk('', '');
  ...
  try { at = new Date(statSync(join(repo, '.git', 'packed-refs')).mtimeMs).toISOString(); } catch { /* none */ }
...
export function headCommit(repo: string): string | null {
  ...
  if (direct) return head.slice(0, 7);
...
export function hasReleaseTag(repo: string): boolean {
  try {
    if (readdirSync(join(repo, '.git/refs/tags')).length) return true;
  } catch { /* no loose tags */ }
  return /(^|\n)[0-9a-f]{40}\s+refs\/tags\//.test(read(repo, '.git/packed-refs'));
}

/** Is there a git repo here at all? */
export const isRepo = (repo: string): boolean => existsSync(join(repo, '.git'));
