...
export const isPackageDir = (abs: string): boolean => MANIFESTS.some(m => existsSync(join(abs, m)));
 ...
 */
export function workspaceGlobs(repo: string): string[] {
  const globs: string[] = [];
  ...
  for (const file of ['melos.yaml', 'pnpm-workspace.yaml']) {
    ...
    for (const raw of text.split('\n')) {
      if (/^packages\s*:/.test(raw)) { inList = true; continue; }
      if (!inList) continue;
  ...
  try {
    ...
    else if (w && Array.isArray(w.packages)) globs.push(...w.packages);
 ...
 */
export function expandGlob(repo: string, glob: string, maxStar = 3): string[] {
  const parts = glob.split('/').filter(Boolean);
  const out: string[] = [];

  const walk = (rel: string, i: number, starBudget: number): void => {
    if (i === parts.length) {
      if (rel && isPackageDir(join(repo, rel))) out.push(rel);
      return;
    }
    const part = parts[i];
    ...
    if (part === '**') {
      ...
      for (const n of children) {
        ...
        if (starBudget > 0) walk(child, i, starBudget - 1);
      }
      return;
    }
    if (part === '*') {
      for (const n of children) walk(rel ? `${rel}/${n}` : n, i + 1, starBudget);
      return;
    }
    const child = rel ? `${rel}/${part}` : part;
    if (isDir(join(repo, child))) walk(child, i + 1, starBudget);
  };
...
export const isWorkspace = (repo: string): boolean =>
 ...
 */
export function workspacePackageDirs(repo: string): string[] {
  if (!isWorkspace(repo)) return [];
