...
import type { Host } from './drain.js';
import type { NightshiftConfig } from './config.js';
import { dirOf, parseTask, taskPath, writeTask, type Task } from './backlog.js';
import { buildGroomPrompt, type GroomHost } from './groom.js';
import { readRunState, writeRunState, type Phase } from './runstate.js';

/**
 * The real Host: the only place Nightshift touches processes, git and the
 * network. Everything that decides anything lives in drain/policy/outcome and
 * is tested against a fake; this file is deliberately dumb so that the parts
 * which are hard to test are also the parts with no judgement in them.
 */

export interface HostOptions {
  repo: string;
  config: NightshiftConfig;
  /** Agent binary. Defaults to `claude` on PATH. */
  agentBin?: string;
  logFile?: string;
  /** Clock. Injected so the host and the caller cannot disagree about "now" —
   *  they did, and the drain read a stale deadline as an already-closed window. */
  now?: () => Date;
  /**
   * Retry backoff. Injectable for the same reason `agentBin` is: the real one
   * blocks the thread for minutes, so a test that reaches the retry path
   * without overriding it does not fail — it hangs, which is far harder to
...
export function makeHost(o: HostOptions): Host & GroomHost {
  const { repo, config } = o;
  ...
  const isPreserved = (f: string) => preserved.some(p => f.startsWith(p));
  ...
  const git = (...args: string[]): string => gitRaw(...args).trim();
  ...
  const untracked = (): string[] =>
    gitRaw('ls-files', '--others', '--exclude-standard').split('\n').map(l => l.trim()).filter(Boolean);
   ...
   */
  let foreignUntracked = new Set<string>();
   ...
   */
  let beforeGroom = new Set<string>();
   ...
   */
  let beforeAgent = new Set<string>();
  ...
  return {
     ...
     */
    setPhase(phase: Phase, task?: Task) {
    ...
    writeTask: (t: Task) => writeTask(repo, t, config.backlogDir),
     ...
     */
    runHook(command: string, timeoutS: number) {
      const started = Date.now();
     ...
     */
    runGate() {
      if (!config.gate.trim()) return false;
    ...
    commit(message: string, paths: string[]) {
      ...
      if (!gitOk('add', '-A', '--', ...paths)) return '';
      if (!gitOk('commit', '-m', message)) return '';   // nothing staged → no commit
    ...
    publish(mode, branch) {
      ...
      if (!gitOk('push', '-u', 'origin', branch)) { log(`push of ${branch} FAILED`); return false; }
      ...
      const pr = spawnSync('gh', ['pr', 'create', '--fill', '--base', config.baseBranch, '--head', branch],
        { cwd: repo, encoding: 'utf8' });
      if (pr.status !== 0) log(`branch ${branch} pushed, but opening a PR failed — open it by hand`);
      return true;
    },
    ...
    revert() {
      ...
      if (dirty.length) gitOk('checkout', '--', ...dirty);
      const created = untracked().filter(f => !foreignUntracked.has(f) && !isPreserved(f));
      if (created.length) gitOk('clean', '-fd', '--', ...created);
    },
    ...
    checkoutBase(branch: string) {
      ...
      // a bookkeeping step.
      if (!gitOk('checkout', branch)) log(`could not return to ${branch}`);
    },

    log,
    sleep: o.sleep ?? ((seconds: number) => {
 ...
 */
export function buildPrompt(repo: string, config: NightshiftConfig, task: Task): string {
  return [
