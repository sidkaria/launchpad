...
import { liveness, readRunState, writeRunState, type Phase } from './runstate.js';
import {
  renderText, renderHtml, subject,
  type GroomSummary, type HookResult, type NightReport, type TaskResult,
} from './report.js';

/**
 * The entry point the scheduler fires, every 30 minutes through the night.
 *
 * Most ticks do nothing, and that is the design: the window, the enabled flag
 * and the lock are all re-checked here rather than encoded in the schedule, so
 * a machine that was asleep at 23:00 still works when it wakes at 01:00.
 */

export interface RunOptions {
  home?: string;
  now?: Date;
  /** Written by a previous tick that is still running. */
  lockPath?: string;
  /**
   * Agent binary. MUST be injectable: without it a test that reaches the drain
   * spawns the real agent against a temp repo, which is slow, costs money, and
   * is not something a test suite should ever be able to do by omission.
   */
  agentBin?: string;
  /** Retry backoff. Overridden by tests so the retry path cannot hang a suite. */
  sleep?: (seconds: number) => void;
}

export interface RunSummary {
  report: NightReport;
  /** Projects considered and what happened to each. */
  skipped: { project: string; why: string }[];
  /**
   * Projects that actually ran, name → repo path.
   *
   * Carried out of here because the caller has to write each project's report
   * INTO that project. It previously only had names, so every report was
   * written to the process working directory — which for a scheduled job is
   * not a repo at all, and the write failed silently into a catch. The morning
   * report existed in memory and nowhere else.
   */
  ran: { project: string; path: string }[];
}

export function runNight(opts: RunOptions = {}): RunSummary {
  const now = opts.now ?? new Date();
  const registry = readRegistry(opts.home);
  const skipped: { project: string; why: string }[] = [];
  const results: TaskResult[] = [];
  const ran: { project: string; path: string }[] = [];
  const hooks: HookResult[] = [];
  const groomed: GroomSummary[] = [];
  ...
  for (const { path, config } of configs) {
    ...
    if (!config?.enabled) { skipped.push({ project: name, why: 'disabled' }); continue; }

    const r = readiness(config);
    if (!r.ok) { skipped.push({ project: name, why: r.problems.join('; ') }); continue; }
    if (!inWindow(config, now)) { skipped.push({ project: name, why: `outside the ${config.window} window` }); continue; }
    ...
    for (const change of out.statusChanges) setStatus(path, change.id, change.status, change.reason, config.backlogDir);
    results.push(...out.results);
    hooks.push(...out.hooks);
    if (out.halted) { halted = `${name}: ${out.halted}`; }
    ...
    if (rs) writeRunState(path, { ...rs, phase: 'finished' as Phase, updatedAt: new Date().toISOString(), finishedAt: new Date().toISOString(), taskId: undefined, taskTitle: undefined });
  }
...
function windowEnd(window: string, now: Date): Date {
  const m = /-(\d{1,2}):(\d{2})$/.exec(window);
  if (!m) return new Date(now.getTime() + 8 * 3600_000);
  const end = new Date(now);
  end.setHours(Number(m[1]), Number(m[2]), 0, 0);
  if (end <= now) end.setDate(end.getDate() + 1);   // the window wraps past midnight
  return end;
}

/** Append-only history, in the project's repo. The morning report reads it. */
function appendEvents(repo: string, results: TaskResult[], now: Date): void {
 ...
 */
export function writeReport(dir: string, report: NightReport): { text: string; html: string } {
 ...
 */
export function writeReports(summary: RunSummary, home = launchpadHome()): { text: string; html: string } {
...
export function eventsOn(repo: string, date: string): TaskResult[] {
  const p = join(repo, '.launchpad', 'nightshift', 'events.jsonl');
  if (!existsSync(p)) return [];
  try {
    return readFileSync(p, 'utf8').split('\n').filter(Boolean)
      .map(l => JSON.parse(l) as TaskResult & { ts: string })
