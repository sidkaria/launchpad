...
export type Scheduler = 'launchd' | 'schtasks' | 'systemd';
...
export const agentPath = (home: string): string =>
  AGENT_PATH_DIRS.map(d => d.replace(/^~/, home)).join(':');

export interface ScheduleSpec {
  /** Fires every `intervalMinutes` between these local times. */
  startHour: number;
  startMinute: number;
  intervalMinutes: number;
  /** Absolute path to the node binary and to the drain entry point. */
  node: string;
  script: string;
  /** Where logs go. */
  logDir: string;
...
export function schedulerFor(platform: NodeJS.Platform): Scheduler {
...
export function launchdPlist(s: ScheduleSpec, home = '~'): string {
 ...
 */
export function schtasksCommand(s: ScheduleSpec): string {
  const label = s.label ?? DEFAULT_LABEL;
...
export function systemdUnits(s: ScheduleSpec, home = '~'): { service: string; timer: string } {
  const label = s.label ?? DEFAULT_LABEL;
  return {
    service: [
      '[Unit]',
...
export interface InstallPlan {
  scheduler: Scheduler;
  /** Files to write, path → contents. Paths are absolute. */
  files: Record<string, string>;
  /** Commands to run, in order, to activate it. */
  commands: string[];
  /** How to undo it. */
  uninstall: string[];
}

export function installPlan(platform: NodeJS.Platform, s: ScheduleSpec, home: string): InstallPlan {
  const label = s.label ?? DEFAULT_LABEL;
...
export function sleepAdvice(platform: NodeJS.Platform): string {
  switch (schedulerFor(platform)) {
    case 'launchd':
