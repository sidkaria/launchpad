...
export interface Release {
  version: string;
  /** Where the package can be downloaded. Shown, never fetched automatically. */
  url?: string;
  /** ISO date. */
  published?: string;
  ...
  notes?: string[];
  /**
   * A version below which this release is not optional — a data-loss fix, a
   * security fix. Said louder, still never installed for them.
   */
...
export type UpdateCheck =
  ...
  | { state: 'behind'; from: string; to: Release; critical: boolean }
  /** Ahead of the manifest: a dev build, or a rollback. Not an error. */
  | { state: 'ahead'; version: string }
...
function coerce(raw: unknown): Release | null {
  if (!raw || typeof raw !== 'object') return null;
  const r = raw as Partial<Release>;
  if (typeof r.version !== 'string' || !/^\d/.test(r.version.replace(/^v/, ''))) return null;
  return {
    version: r.version,
    url: typeof r.url === 'string' ? r.url : undefined,
    published: typeof r.published === 'string' ? r.published : undefined,
    notes: Array.isArray(r.notes) ? r.notes.filter((n): n is string => typeof n === 'string').slice(0, 20) : undefined,
    criticalBelow: typeof r.criticalBelow === 'string' ? r.criticalBelow : undefined,
  };
}

export async function checkForUpdate(
  current: string, fetcher: Fetcher, url = MANIFEST_URL,
): Promise<UpdateCheck> {
  let release: Release | null;
  try {
    const res = await fetcher(url);
    if (!res.ok) return { state: 'unknown', why: `the update server answered ${res.status}` };
    ...
    return { state: 'unknown', why: (e as Error).message };
  }
  if (!release) return { state: 'unknown', why: 'the update server sent something unreadable' };

  const cmp = compareVersions(current, release.version);
  if (cmp === 0) return { state: 'current', version: current };
  if (cmp > 0) return { state: 'ahead', version: current };
  return {
    state: 'behind',
    from: current,
    to: release,
...
export function renderUpdate(c: UpdateCheck): string {
  switch (c.state) {
    case 'current':
    ...
    case 'unknown':
      ...
      return `launchpad: could not check for updates (${
        /fetch failed|ECONNREFUSED|ENOTFOUND|EAI_AGAIN|network/i.test(c.why) ? 'no connection'
    ...
    case 'behind': {
      ...
      if (c.to.published) out.push(`  published ${c.to.published.slice(0, 10)}`);
      if (c.to.notes?.length) {
        out.push('');
        for (const n of c.to.notes.slice(0, 8)) out.push(`  · ${n}`);
      }
      out.push('');
...
export const realFetcher: Fetcher = async (url: string) => {
