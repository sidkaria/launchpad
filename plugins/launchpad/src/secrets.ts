...
import { iosSecrets, isWireable, IOS_CLOUD_SECRETS, IOS_MATCH_SECRETS, type IosConfig } from './archetypes/ios.js';
import { MACOS_GLOBAL_SECRETS, MACOS_PER_APP_SECRETS } from './archetypes/macos.js';
import { ANDROID_GLOBAL_SECRETS, androidFramework, androidIsWireable, type AndroidConfig } from './archetypes/android.js';
 ...
 */
const VAULT_KEY_OVERRIDES: Record<string, string> = {
  ASC_KEY_P8: 'asc_api_key',            // the vault holds the raw .p8 as asc_api_key
  CLOUDFLARE_API_TOKEN: 'cloudflare_token',
};

export function vaultKeyFor(secretName: string): string {
  return VAULT_KEY_OVERRIDES[secretName] ?? secretName.toLowerCase();
}

export interface RequiredSecrets {
  // Resolvable from the global vault via vaultKeyFor — these auto-inject.
  global: string[];
  // Per-app secrets (e.g. macOS SPARKLE_PRIVATE_KEY) live under an app-slug
  // prefix, not a global key, so they're surfaced rather than auto-injected.
  perApp: string[];
}

export interface SecretGroup {
  label: string;                // what needs these, in human terms
  secrets: readonly string[];   // GitHub secret names
}
 ...
 * Android.
 */
export const SECRET_CATALOGUE: readonly SecretGroup[] = [
...
export const PER_APP_SECRETS: readonly string[] = MACOS_PER_APP_SECRETS;
 ...
 */
export function requiredVaultKeys(state: LaunchpadState): string[] {
...
export function requiredSecrets(state: LaunchpadState): RequiredSecrets {
  const global = new Set<string>();
  const perApp = new Set<string>();
  const add = (set: Set<string>, names: readonly string[]) => names.forEach(n => set.add(n));
  for (const s of state.surfaces) {
    ...
    if (s.archetype === 'android' && !androidIsWireable(androidFramework(s.config as Partial<AndroidConfig> | undefined, s))) continue;
    if (s.archetype === 'ios' && s.framework && !isWireable(s.framework)) continue;
    switch (s.archetype) {
      case 'ios':
        ...
        add(global, s.config ? iosSecrets(s.config as IosConfig) : IOS_CLOUD_SECRETS);
        break;
      case 'macos-dmg':
