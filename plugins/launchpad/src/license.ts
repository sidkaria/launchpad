...
import type { Fetcher, LicenseProvider } from './providers/types.js';
 ...
 */
export const LICENSED_VERBS = ['apply', 'secrets', 'release', 'domains'] as const;

export interface LicenseRecord {
  key: string;
  /** Provider instance/activation id, from activation. Needed to deactivate. */
  instanceId?: string;
  /**
   * Which rail issued this key — `lemonsqueezy`, `polar`.
   *
   * Recorded so a later build that defaults to a different provider re-checks
   * the key against the one that actually knows it. Absent on records written
   * before this field existed, which were all Lemon Squeezy.
   */
  provider?: string;
  /** ISO. Last time the provider said yes. */
  validatedAt: string;
  /** ISO. Last time we tried, successfully or not. */
  checkedAt?: string;
  /** What the provider last told us, when it told us anything. */
  lastError?: string;
}

export type Entitlement =
  | { state: 'licensed'; since: string }
  /**
   * Validated once, not re-checked lately — usually because the machine has
   * been offline. Fully entitled, with no end date. Reported so that `license`
   * can explain itself, never so that anything is withheld.
   */
  | { state: 'stale'; offlineDays: number; why: string }
  /** The provider positively said no. The only way to lose entitlement. */
  | { state: 'lapsed'; why: string }
  | { state: 'unlicensed' };

export const licensePath = (home = homedir()): string => join(home, '.launchpad', 'license.json');
 ...
 */
export function readLicense(home = homedir()): LicenseRecord | null {
  const p = licensePath(home);
  if (!existsSync(p)) return null;
  try {
    const raw = JSON.parse(readFileSync(p, 'utf8')) as Partial<LicenseRecord>;
...
export function writeLicense(rec: LicenseRecord, home = homedir()): void {
  const p = licensePath(home);
...
export function clearLicense(home = homedir()): void {
  try { writeFileSync(licensePath(home), '{}\n', 'utf8'); } catch { /* nothing to clear */ }
}
 ...
 */
export function entitlement(rec: LicenseRecord | null, now: Date, staleAfterDays = STALE_AFTER_DAYS): Entitlement {
  if (!rec) return { state: 'unlicensed' };
  ...
  if (!(days > staleAfterDays)) return { state: 'licensed', since: rec.validatedAt };
  return {
    state: 'stale',
 ...
 */
export function licenceWord(state: Entitlement['state']): string {
 ...
 */
export const isEntitled = (e: Entitlement): boolean => e.state === 'licensed' || e.state === 'stale';

export type { Fetcher, LicenseProvider } from './providers/types.js';
...
export async function activate(
  ...
  provider: LicenseProvider = selectProvider(key),
): Promise<ActivateResult> {
  const trimmed = key.trim();
  if (!trimmed) return { ok: false, message: 'A licence key is required.' };

  const stored = (instanceId?: string): LicenseRecord => ({
    key: trimmed,
    instanceId,
  ...
  if (r.status === 'unreachable') return { ok: false, message: unreachableMessage(provider.label, r.why) };
  if (r.status === 'refused') return { ok: false, message: r.why };
 ...
 */
export function humanReason(why: string): string {
  if (/aborted|abort|timed? ?out|timeout/i.test(why)) return 'no answer in time';
  ...
  if (http) {
    ...
    if (code === 429) return 'it is rate-limiting requests right now';
    if (code >= 500) return `it is having an outage (HTTP ${code})`;
    return `HTTP ${code}`;
  }
  return why;
}
...
export function unreachableMessage(label: string, why: string): string {
...
export async function revalidate(
  ...
  now = new Date(),
  signal?: AbortSignal,
  provider: LicenseProvider | null = providerForRecord(rec),
): Promise<{ record: LicenseRecord; reachable: boolean }> {
  ...
  if (v.status === 'unreachable') return unreachable(v.why);
  if (v.status === 'valid') {
    return {
      record: {
        ...rec,
...
export async function refreshIfStale(
  ...
  now = new Date(),
  opts: { afterHours?: number; timeoutMs?: number; provider?: LicenseProvider | null } = {},
  ...
  try {
    ...
    if (!Number.isNaN(since) && since < wait) return rec;
...
export async function deactivate(
  ...
  if (!rec.instanceId) return { ok: false, message: 'No activation on this machine to release.' };
  ...
  if (r.status === 'released') return { ok: true, message: 'Released this machine. The seat is free for another.' };
...
export function refusal(
  verb: string, e: Entitlement, checkoutUrl: string, supportUrl?: string,
  providerName = 'The licence provider', price?: string,
): string {
  if (e.state === 'lapsed') {
    /**
