...
import { makeVault, detectBackend, requestedBackend, backendLabel, vaultReadCommand } from '../vault.js';
...
import { writeIosFiles, iosSecrets, isWireable } from '../archetypes/ios.js';
import { writeAndroidFiles, ANDROID_GLOBAL_SECRETS, androidFramework, androidIsWireable, androidRefusal, } from '../archetypes/android.js';
...
import { activate, deactivate, revalidate, refreshIfStale, readLicense, writeLicense, clearLicense, entitlement, isEntitled, refusal, realFetcher, providerLabel, LICENSED_VERBS, } from '../license.js';
...
function cmdApply(repo) {
    const st = readState(repo);
    if (!st) {
        console.error('launchpad: run setup first');
        process.exit(1);
        return;
    }
    let wired = 0;
    ...
    if (wired > 0)
        console.log('Next: `launchpad secrets` injects these into the repo from your Keychain vault (no hand-typing).');
}
...
function cmdSecret(key) {
    if (!key) {
        console.error('usage: launchpad secret <vault-key>   (e.g. cloudflare_token)');
        process.exit(2);
        return;
    }
...
async function cmdLicense(sub, key) {
    ...
    if (sub === 'activate') {
        if (!key) {
            console.error('launchpad: license activate <key>');
            process.exit(1);
            return;
        }
    ...
    if (sub === 'deactivate') {
        if (!rec) {
            console.log('launchpad: no licence stored on this machine.');
            return;
        }
...
switch (cmd) {
    ...
    case 'projects':
        ...
        break;
    case 'confirm':
        cmdConfirm(repo, process.argv[3], process.argv.slice(4).filter(a => a !== '--undo').join(' ') || undefined, process.argv.includes('--undo'));
        break;
    case 'dashboard':
        cmdDashboard(process.argv.slice(3)).catch((e) => { console.error(e); process.exit(1); });
        break;
    case 'version':
        console.log(renderVersion(prov));
        break;
    case 'report':
        cmdReport(repo, process.argv.slice(3).find(a => a.startsWith('--out='))?.slice(6));
        break;
    case 'update':
        ...
        break;
    case 'license':
        cmdLicense(process.argv[3], process.argv[4]).catch((e) => { console.error(e); process.exit(1); });
        break;
    case 'nightshift':
        cmdNightshift(repo, process.argv[3], process.argv.slice(4));
        break;
    case 'add':
        cmdAdd(repo, process.argv[3]);
        break;
    case 'remove':
        cmdRemove(repo, process.argv[3]);
        break;
    case 'secrets':
        cmdSecrets(repo, process.argv.includes('--dry-run'));
        break;
    case 'secret':
        cmdSecret(process.argv[3]);
        break;
    case 'release':
        cmdRelease(repo, process.argv[3] ?? '', process.argv[4] ?? '', process.argv[5] ?? 'v');
        break;
    case 'domains': {
