 ...
 */
export function pipelineArchetype(p) {
 ...
 */
export function pipelineScope(p) {
    const path = p.path.replace(/\\/g, '/');
...
export const surfaceScope = (s) => s.target ?? '.';
/**
 ...
 */
export function blockingPipeline(s, pipelines) {
    return pipelines.find(p => p.disposition === 'leave-alone'
 ...
 */
export const DEFAULT_DISPOSITION = 'leave-alone';
/** The refusal `apply` prints. Names the pipeline, and how to say otherwise. */
export function dispositionRefusal(s, p) {
    return [
