 ...
 * stays parseable.
 */
const ASSIGNMENT = /\b([A-Za-z_][A-Za-z0-9_.-]*(?:secret|token|password|passphrase|key|credential|auth)[A-Za-z0-9_.-]*)(\s*[=:]\s*)(\S+)/gi;
/**
 ...
 */
export function tildeHome(text, home) {
    if (!home)
        return text;
...
export function redact(text, home = '') {
    ...
    for (const [pattern, replacement] of SECRET_VALUE)
 ...
 */
export function redactTree(value, home = '', depth = 0) {
    ...
    if (typeof value === 'string')
        return redactFreeText(value, home);
    if (typeof value === 'number' || typeof value === 'boolean')
        return value;
    if (Array.isArray(value))
        return value.map(v => redactTree(v, home, depth + 1));
 ...
 * channel.
 */
export function shortenPath(path, home = '') {
 ...
 * nothing at all.
 */
export const keyTail = (key) => key && key.length ? `…${key.slice(-4)}` : 'none';
