...
export const realProbe = (bin, args) => {
    try {
...
function probeTools(probe, home) {
...
function tailLogs(repo, home, lines) {
    ...
    for (const name of ['nightshift.out.log', 'nightshift.err.log']) {
        ...
        try {
            ...
            if (tail.length)
                out.push({ file: join('.launchpad', 'nightshift', name), tail });
        }
        catch {
...
export function buildReport(deps) {
    ...
    const vaultHas = deps.vaultHas ?? ((key) => {
        try {
            return makeVault().has(key);
        }
        catch {
            return false;
        }
    ...
    try {
        if (existsSync(statePath(repo)))
            rawState = parse(readFileSync(statePath(repo), 'utf8'));
    }
    catch {
        rawState = null;
    }
 ...
 */
export function renderReport(r, home = '') {
    const out = [];
    ...
    if (r.capabilities.length) {
        ...
        for (const c of r.capabilities) {
            ...
            for (const s of c.macOnlySetup)
                p(`  - needs a Mac once: ${s}`);
        }
        p();
    }
    p('## Tools');
    ...
    for (const t of r.tools)
        p(`- ${t.name}: ${t.version ?? '_not installed_'}`);
    p();
    p('## Project');
    p();
    p(`| | |`);
    ...
    if (r.project.pipelines.length) {
        ...
        p();
        for (const pl of r.project.pipelines)
            p(`- ${pl.kind} at \`${pl.path}\` — disposition: ${pl.disposition}`);
        p();
    }
    p('## Credentials');
    p();
    if (!r.credentials.length) {
        ...
        for (const c of r.credentials)
            p(`- ${c.present ? '✓ present' : '✗ MISSING'} — \`${c.name}\``);
    }
    p();
    if (r.scorecard) {
        p('## Scorecard');
