...
export function scorecard(repo, st) {
    ...
    if (kinds.has('android')) {
        const gradle = read(repo, 'android/app/build.gradle.kts') || read(repo, 'android/app/build.gradle')
            || read(repo, `${(configOf(st, 'android')?.workdir ?? '.')}/android/app/build.gradle.kts`);
        add({
            id: 'android-signing', title: 'Android release is signed with a real key',
            ...grade(/signingConfigs[\s\S]*release/.test(gradle), 'A stock Flutter or Android project signs its RELEASE build with the DEBUG keystore. ' +
