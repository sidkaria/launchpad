...
const RULES = [
    ...
    },
    {
        ...
        detect: r => {
            ...
            if (/"electron"\s*:/.test(pkg))
                return 'electron in package.json';
            if (/"electron-builder"\s*:|"@electron-forge\//.test(pkg))
                return 'electron-builder in package.json';
    ...
    },
    {
        ...
        detect: r => {
            ...
            if (has(r, 'manage.py') && /django/i.test(deps + read(r, 'manage.py')))
                return 'manage.py + Django';
 ...
 */
export const ECOSYSTEM_CATALOGUE = RULES.map(({ id, name, ships, registry }) => ({ id, name, ships, ...(registry ? { registry } : {}) }));
/**
 ...
 */
export function ecosystemOf(repo) {
 ...
 */
export function refine(repo, eco) {
    if (eco.id === 'rust') {
        ...
        if (isBin)
            return { ...eco, name: 'Rust CLI', ships: 'cli' };
        if (isLib)
            return { ...eco, name: 'Rust crate', ships: 'library' };
    }
    if (eco.id === 'go') {
...
export const detectEcosystem = (repo) => {
...
export function installPhrase(eco) {
    return eco.registry
