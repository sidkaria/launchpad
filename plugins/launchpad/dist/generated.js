...
function stampLine(relPath, hash) {
    const text = `launchpad:generated ${hash} — edits here are preserved, but re-running \`apply\` will tell you they exist`;
    if (/\.(ya?ml|sh|bash|zsh|rb|toml|conf|properties|gradle)$/.test(relPath))
        return `# ${text}`;
    if (HASH_COMMENT_BASENAMES.has(basename(relPath)))
        return `# ${text}`;
    if (/\.(swift|kt|kts|ts|js|mjs|cjs|dart|java|c|cc|cpp|h|m)$/.test(relPath))
        return `// ${text}`;
 ...
 * not a migration.
 */
const stampable = (relPath) => stampLine(relPath, '0'.repeat(16)) !== null;
/** Insert the stamp after a shebang, which must stay on line one. */
function withStamp(relPath, contents) {
...
const withoutStamp = (contents) => contents.split('\n').filter(l => !STAMP.test(l)).join('\n');
/**
 ...
 */
export function userEdited(contents) {
 ...
 */
export function isLaunchpadGenerated(relPath, contents) {
    if (basename(relPath).startsWith('launchpad-'))
        return true;
 ...
 */
export function writeGuarded(repo, files) {
    const written = [];
    const preserved = [];
    const edited = [];
    const unverified = [];
