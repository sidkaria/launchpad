...
export function cliInvocation(path = cliPath(), home = loginHome(), platform = process.platform) {
    const abs = resolve(path);
    if (platform !== 'win32' && home) {
        const h = resolve(home);
        if (abs.startsWith(h + sep))
            return `node "$HOME/${abs.slice(h.length + 1).split(sep).join('/')}"`;
    }
    return `node "${abs}"`;
}
/** `launchpad secret set x` → `node "$HOME/…/dist/cli/index.js" secret set x`. */
export function runnable(command, invocation = cliInvocation()) {
