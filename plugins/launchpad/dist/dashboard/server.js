 ...
 */
export function demoLabel(home) {
...
async function handle(req, res, home, token, demo = false) {
    ...
    if (req.method === 'GET' && url.pathname === '/api/icon') {
        ...
        try {
            ...
            if (buf.length > 8 * 1024 * 1024)
                return json(res, 413, { error: 'icon too large' });
            res.writeHead(200, {
    ...
    if (req.method === 'POST') {
        ...
        if (url.pathname === '/api/projects/add') {
            ...
            if (!raw)
                return json(res, 400, { error: 'a path is required' });
            ...
            if (!isAbsolute(expanded))
                return json(res, 400, { error: 'give an absolute path' });
        ...
        switch (url.pathname) {
            case '/api/idea': {
                ...
                if (!text)
                    return json(res, 400, { error: 'an idea needs some words' });
                addIdea(repo, text);
                return json(res, 200, { ok: true });
            }
            /**
            ...
            case '/api/confirm': {
                ...
                if (!check)
                    return json(res, 400, { error: 'which check?' });
                const st = readState(repo);
                if (!st)
                    return json(res, 400, { error: 'this project is not set up' });
                if (body.confirmed === false) {
                    unconfirm(repo, check);
                    return json(res, 200, { ok: true });
                }
                const target = scorecard(repo, st).checks.find(c => c.id === check);
                if (!target)
                    return json(res, 400, { error: `no check called ${check}` });
            ...
            case '/api/disposition': {
                ...
                if (!pipe)
                    return json(res, 400, { error: `no pipeline at ${path} in this project` });
                pipe.disposition = to;
                writeState(repo, st);
                return json(res, 200, { ok: true });
            }
            case '/api/nightshift/enabled': {
                ...
                if (!config)
                    return json(res, 400, { error: 'nightshift is not set up in this project' });
            ...
            case '/api/task/create': {
                ...
                if (!title)
                    return json(res, 400, { error: 'a task needs a title' });
                ...
                const config = readConfig(repo);
                const t = createTask(repo, { title, body: String(body.body ?? ''), status, priority }, new Date(), config?.backlogDir);
