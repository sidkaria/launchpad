...
function surface(id, archetype, evidence, confidence, framework) {
...
function detectWebFramework(_dir, pkg, has) {
 ...
 * as before.
 */
export const STATIC_GENERATORS = [
 ...
 */
export function detectStaticGenerator(dir, has = rel => existsSync(join(dir, rel))) {
    for (const g of STATIC_GENERATORS) {
        const hit = g.files.find(has);
        if (!hit)
            continue;
        if (g.requires && !g.requires.test(read(join(dir, hit))))
            continue;
        return g.name;
    }
    return null;
 ...
 */
function hostedWebEvidence(dir) {
...
function scanDir(dir, at) {
    ...
    if (xcodeprojDirs.length || has('project.yml') || has('Package.swift')) {
        ...
        if (/macos|macosx|\.macOS/i.test(blob))
            out.push(surface('macos', 'macos-dmg', ['Apple project targets macOS'], 'low'));
        if (/iphoneos|\.iOS|platform:\s*iOS/i.test(blob))
            out.push(surface('ios', 'ios', ['Apple project targets iOS'], 'low', 'native'));
    }
    // Flutter
    if (has('pubspec.yaml')) {
        const ios = has('ios');
        ...
        if (ios)
            out.push(surface('ios', 'ios', ['Flutter ios/ present'], 'high', 'flutter'));
        if (android)
            out.push(surface('android', 'android', ['Flutter android/ present'], 'high', 'flutter'));
    ...
    if (!has('pubspec.yaml') && !isExpoEarly && !isRNEarly) {
        ...
        if (appModule !== null) {
            ...
            out.push(surface('android', 'android', [`Android Gradle application plugin in ${where}`], 'high', 'native'));
        }
        else {
            ...
            if (weak)
                out.push(surface('android', 'android', [weak], 'low', 'native'));
        }
    }
    ...
    if (isExpo || isRN) {
        ...
        const framework = isExpo ? 'expo' : 'react-native';
    ...
    if (webFramework) {
        ...
        if (!shell || hosted) {
            out.push(surface('web', 'web-app', [`${webFramework} detected`, ...(hosted ? [`hosted web target (${hosted})`] : [])], 'high'));
        }
        else {
            skipped.push({
                path: at,
    ...
    if (ssg)
        out.push(surface('site', 'static-site', [`${ssg} detected`], 'high'));
...
function pipelinesIn(dir, pathPrefix) {
    const out = [];
    const has = (rel) => existsSync(join(dir, rel));
    const rel = (p) => (pathPrefix ? join(pathPrefix, p) : p);
     ...
     */
    const foreign = (relPath) => !isLaunchpadGenerated(relPath, read(join(dir, relPath)));
    /**
    ...
    if (sparkle && foreign(sparkle))
        out.push({ kind: 'sparkle-dmg', path: rel(sparkle) });
    if (has('vercel.json') && foreign('vercel.json'))
        out.push({ kind: 'vercel', path: rel('vercel.json') });
    if (has('.heartbeat') && foreign('.heartbeat'))
        out.push({ kind: 'heartbeat', path: rel('.heartbeat') });
    return out;
}
/**
 ...
 */
function notShippable(repo, relDir) {
...
export function detect(repo) {
    ...
    const pipelines = [...pipelinesIn(repo, '')];
