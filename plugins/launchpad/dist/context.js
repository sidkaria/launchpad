 ...
 */
export function renderBlock(state, opts = {}) {
    ...
    if (ns) {
        lines.push(`| \`${ns.backlogDir}/*.md\` | the roadmap | One file per task. **Update \`status:\` the moment it changes.** |`, '| `.launchpad/nightshift/inbox.md` | un-triaged ideas | Append-only; grooming drains it. |', '| `.launchpad/nightshift/run.json` | what is running now | Written by the worker. Do not touch. |', '', '**If you work on a task from the backlog, you own its status.**', '', `- Starting it → \`status: in_progress\`. A crash then leaves a trail instead of losing the work.`, `- Finished and \`${ns.gate}\` passes → \`status: done\`.`, `- Cannot finish it → \`status: blocked\` **with a \`reason:\`**. A blocked task with no reason is`, '  indistinguishable from one the overnight worker killed by accident, and it will sit there forever.', '- Not actually started → leave it alone. `ready` is not a lie you need to tell.', '', 'A task finished without its status moved gets picked up and built a second time tonight.', '', `The gate is \`${ns.gate}\`. It must exit 0 before anything is committed, and **it is never**`, '**edited to make it pass** — it is the only thing standing between unattended work and', 'code nobody checked.');
        if (ns.protectedPaths.length) {
            lines.push('', `Never modify: ${ns.protectedPaths.map(p => `\`${p}\``).join(', ')}.`);
        }
    }
    lines.push('', 'After changing what this project *is* — a new surface, a renamed app, a new platform —', 're-run `/launchpad:setup`. Detection is re-run and the scorecard re-derives; nothing else', 'needs telling.', '', END);
    return lines.join('\n');
}
/**
 ...
 * had just installed.
 */
export function upsertClaudeMd(repo, state, nightshift) {
 ...
 */
export function writeDeploymentDoc(repo, state) {
