...
const web = (id, name, detectedAs, icon, aliases) => ({
...
const static_ = (id, name, detectedAs, icon, aliases) => ({
...
const known = (id, name, category, icon, shipsTo, aliases) => ({
...
export const PLATFORMS = [
    ...
    known('tauri', 'Tauri desktop app', 'desktop', 'tauri', 'A signed installer per OS, built by its own toolchain.', ['rust', 'desktop', 'webview']),
    known('electron', 'Electron desktop app', 'desktop', 'electron', 'A signed installer per OS, built by electron-builder or Forge.', ['electron-builder', 'forge', 'desktop']),
    known('swift-package', 'Swift package', 'library', 'swift', 'A version tag other packages can depend on — SwiftPM resolves straight from the git URL.', ['spm', 'swiftpm', 'package.swift', 'swift package manager', 'library']),
    known('rails', 'Ruby on Rails app', 'backend', 'rubyonrails', 'A host that runs it — Fly, Render, Heroku, your own box.', ['ruby', 'gemfile', 'puma']),
    known('django', 'Django app', 'backend', 'django', 'A host that runs it — Fly, Render, Railway, your own box.', ['python', 'manage.py', 'wsgi']),
    known('python-service', 'Python web service', 'backend', 'python', 'A host that runs it — Fly, Render, Railway, your own box.', ['fastapi', 'flask', 'litestar', 'sanic', 'aiohttp', 'uvicorn']),
    known('laravel', 'Laravel app', 'backend', 'laravel', 'A host that runs it — Forge, Vapor, your own box.', ['php', 'artisan', 'composer']),
    known('phoenix', 'Phoenix app', 'backend', 'phoenixframework', 'A host that runs it — Fly, Gigalixir, your own box.', ['elixir', 'mix', 'liveview']),
    known('elixir', 'Elixir project', 'library', 'elixir', 'Hex, so a stranger can add it as a dependency.', ['mix', 'hex', 'erlang', 'beam']),
    known('rust', 'Rust project', 'cli', 'rust', 'crates.io, plus release binaries someone can download.', ['cargo', 'crate', 'crates.io', 'binary']),
    known('go', 'Go project', 'cli', 'go', 'The Go module proxy, plus release binaries someone can download.', ['golang', 'go.mod', 'module', 'binary']),
    known('python', 'Python package', 'library', 'python', 'PyPI, so `pip install` works for a stranger.', ['pypi', 'pip', 'pyproject', 'setup.py', 'wheel']),
    known('ruby-gem', 'Ruby gem', 'library', 'rubygems', 'RubyGems, so `gem install` works for a stranger.', ['gemspec', 'ruby']),
    known('dotnet', '.NET project', 'backend', 'dotnet', 'NuGet, or a host that runs it.', ['c#', 'csharp', 'csproj', 'fsharp', 'nuget', 'asp.net']),
    known('jvm', 'JVM project', 'backend', 'openjdk', 'Maven Central, or a host that runs it.', ['java', 'kotlin', 'gradle', 'maven', 'spring', 'pom.xml']),
    known('node-cli', 'Node CLI', 'cli', 'nodedotjs', 'npm, so `npx` works for a stranger.', ['npm', 'bin', 'javascript', 'typescript']),
    known('node-lib', 'Node package', 'library', 'nodedotjs', 'npm, so `npm install` works for a stranger.', ['npm', 'javascript', 'typescript', 'package.json']),
    known('docker', 'Containerised service', 'backend', 'docker', 'A registry and a host that runs the image.', ['dockerfile', 'compose', 'container', 'kubernetes']),
];
// ── Small helpers. No dependencies, no I/O. ─────────────────────────────────
export const byId = (id) => PLATFORMS.find(p => p.id === id);
export const withStatus = (status) => PLATFORMS.filter(p => p.status === status);
/** Archetypes some entry claims a pipeline for. */
export const deployedArchetypes = () => [...new Set(withStatus('deploys').flatMap(p => p.archetypes ?? []))];
/**
 ...
 */
export const searchIndex = (p) => [p.id, p.name, p.detectedAs ?? '', p.category, ...(p.aliases ?? [])]
    .join(' ')
    .toLowerCase();
...
export const surfaceSentence = () => 'iOS, Android, macOS apps, web apps and static sites';
export const hostNote = (host) => ({
