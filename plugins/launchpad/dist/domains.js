...
export function matchDomain(project, zones) {
 ...
 */
export function zoneForDomain(domain, zones) {
    const d = domain.toLowerCase().replace(/\.$/, '');
    let best = null;
    for (const z of zones) {
        const zn = z.name.toLowerCase();
        if (d === zn || d.endsWith('.' + zn)) {
            if (!best || z.name.length > best.name.length)
                best = z;
        }
    }
    return best;
 ...
 */
export function plannedRecords(domain, zone) {
    const d = domain.toLowerCase().replace(/\.$/, '');
