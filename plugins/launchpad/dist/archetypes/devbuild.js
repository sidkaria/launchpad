...
export function injectDevConfig(projectYml, target, appName, opts) {
    ...
    if (opts?.devIconName)
        debug.ASSETCATALOG_COMPILER_APPICON_NAME = opts.devIconName;
    t.settings.configs.Debug = { ...(t.settings.configs.Debug ?? {}), ...debug };
    return stringify(doc);
}
export function wireDevBuild(repo, c) {
 ...
 */
export function ensureXcodegenScheme(repo, scheme) {
    const pyPath = join(repo, 'project.yml');
    if (!existsSync(pyPath))
        return [];
    const orig = readFileSync(pyPath, 'utf8');
    const doc = parse(orig);
    if (!doc?.targets?.[scheme])
        return []; // scheme must name a real target
    if (doc.schemes?.[scheme])
        return []; // already has a shared scheme
...
export function generateDevIconSvg(baseSvg) {
    const m = baseSvg.match(/viewBox\s*=\s*"\s*([\d.+-]+)[\s,]+([\d.+-]+)[\s,]+([\d.+-]+)[\s,]+([\d.+-]+)\s*"/);
    if (!m)
        throw new Error('generateDevIconSvg: no viewBox found in base SVG');
    const X = parseFloat(m[1]), Y = parseFloat(m[2]), W = parseFloat(m[3]), H = parseFloat(m[4]);
    const band = H * 0.26;
    const overlay = `  <!-- launchpad dev badge -->\n` +
 ...
 * transparent.
 */
export function badgeDevPng(png) {
    ...
    const blend = (i, rgb, a) => {
        for (let k = 0; k < 3; k++)
            rgba[i + k] = Math.round(rgba[i + k] * (1 - a) + rgb[k] * a);
    };
    for (let y = 0; y < H; y++) {
        for (let x = 0; x < W; x++) {
            const i = (y * W + x) * 4;
    ...
    if (cell >= 1) {
        ...
        for (let y = Math.max(0, Math.floor(y0)); y < Math.min(band, Math.ceil(y0 + textH)); y++) {
            ...
            for (let x = Math.max(0, Math.floor(x0)); x < Math.min(W, Math.ceil(x0 + textW)); x++) {
                ...
                if (col < 0 || g >= GLYPHS.length || col % 6 === 5)
                    continue;
                if (GLYPHS[g][row][col % 6] === '1')
                    blend((y * W + x) * 4, [255, 255, 255], 1);
            }
        }
    }
...
function decodePng(buf) {
    ...
    while (off + 12 <= buf.length) {
        ...
        if (type === 'IHDR')
            ...
            idat.push(data);
        else if (KEEP_CHUNKS.has(type))
            keep.push(buf.subarray(off, off + 12 + len));
        off += 12 + len;
        if (type === 'IEND')
            break;
    }
    if (!ihdr || ihdr.length < 13 || !idat.length)
        return null;
    ...
    const ctype = ihdr[9];
    if (ihdr[12] !== 0)
        return null; // Adam7 interlace — copied unbadged
    const channels = { 0: 1, 2: 3, 3: 1, 4: 2, 6: 4 }[ctype];
    if (!channels || !width || !height || width * height > 4096 * 4096)
        return null;
    if (![1, 2, 4, 8, 16].includes(depth))
        return null;
    if (ctype === 3 && (!plte || depth === 16))
        return null;
    ...
    const key = (v) => (depth === 16 ? v >> 8 : depth < 8 ? Math.round((v * 255) / ((1 << depth) - 1)) : v);
...
function encodePng(width, height, rgba, alpha, keep) {
