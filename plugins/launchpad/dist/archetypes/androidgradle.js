...
import { renderOnBlock, scheduleGuardStep, stepIfLine, stepIfVar, pathGlob, repoRelDir, ANDROID_DEFAULT_DISTRIBUTE_ON, ANDROID_DEFAULT_RUNNER, } from './triggers.js';
...
export const GRADLE_FRAMEWORKS = ['native', 'react-native', 'expo'];
export const isGradleFramework = (f) => GRADLE_FRAMEWORKS.includes(f);
/**
...
export const androidSlug = (appName) => appName.replace(/[^A-Za-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '');
/**
 ...
 */
export function gradleDir(c) {
    if (c.gradleDir !== undefined)
        return repoRelDir(c.gradleDir) || '.';
    const base = repoRelDir(c.workdir ?? '.');
    if (c.framework === 'native')
        return base || '.';
    return base ? `${base}/android` : 'android';
}
/** `.` renders as an empty prefix so a path stays repo-root-relative. */
export const gradleDirPrefix = (dir) => (dir === '.' ? '' : `${dir}/`);
/**
...
export function gradleValidateTasks(c) {
 ...
 */
export function validateSteps(c) {
 ...
 */
export function nodeSetupSteps(c, stepIf) {
    if (c.framework !== 'react-native' && c.framework !== 'expo')
        return '';
    ...
    if (pm === 'pnpm') {
        ...
        '      - uses: pnpm/action-setup@v4', ...(stepIf ? [stepIf] : []), '        with:', '          run_install: false', '');
    }
    lines.push('      - name: Install JavaScript dependencies', ...(stepIf ? [stepIf] : []), `        working-directory: ${jsDir}`, `        run: ${installCommand(pm)}`, '', '');
    return lines.join('\n');
}
/**
 ...
 */
export function prebuildStep(c, stepIf) {
    if (c.framework !== 'expo')
        return '';
    const jsDir = repoRelDir(c.workdir ?? '.') || '.';
    return [
...
export const gradleWorkflowPath = (c) => `.github/workflows/launchpad-${androidSlug(c.appName)}-android.yml`;
export const gradleValidateWorkflowPath = (c) => `.github/workflows/launchpad-${androidSlug(c.appName)}-android-validate.yml`;
/** Everything under the app dir — the default scope for the validate lane. */
const defaultValidatePaths = (workdir) => [pathGlob(workdir) || '**'];
export function planGradleAndroidFiles(c) {
    const dir = gradleDir(c);
 ...
 */
export function resolveGradleConfig(repo, c) {
    const dir = gradleDir(c);
