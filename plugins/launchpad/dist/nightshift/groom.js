import { dirOf } from './backlog.js';
import { isExhausted, taskHash } from './groomstate.js';
 ...
 * every listing.
 */
export function planCapture(inbox, existingIds, now) {
    const out = [];
...
const PROMOTABLE = ['ready', 'blocked', 'scoped'];
export function groom(host, opts) {
    const { config } = opts;
    const backlog = dirOf(config.backlogDir).replace(/\/*$/, '/');
    const result = { captured: [], scoped: [], skipped: [], refused: [], succeeded: [], exhausted: [] };
    ...
    if (captured.length)
        host.log(`groom: captured ${captured.length} inbox line(s) as tasks`);
    /**
     ...
     */
    const eligible = [
        ...captured.map(c => ({ id: c.id, status: 'idea', hash: taskHash(c.title, c.body) })),
        ...opts.tasks
    ...
    for (const item of eligible) {
        ...
        if (queue.length < Math.max(0, config.maxGroomPerNight))
            queue.push(item);
    }
    ...
    for (const item of queue) {
        if (host.now() >= opts.deadline) {
            result.halted = 'the window closed during grooming';
            break;
        }
        const before = host.readTask(item.id);
        if (!before) {
            result.skipped.push({ id: item.id, why: 'task file disappeared' });
            continue;
        }
        ...
        if (!after) {
            result.skipped.push({ id: item.id, why: 'task file was deleted' });
            continue;
        }
 ...
 */
export function buildGroomPrompt(repo, taskFile, task) {
    return [
