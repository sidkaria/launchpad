import { describe, reachedRemote } from './outcome.js';
...
export function headline(r) {
    ...
    if (t.committed)
        parts.push(`${t.committed} committed but NOT pushed`);
    if (t.blocked)
        parts.push(`${t.blocked} blocked`);
    if (t.noChange)
        parts.push(`${t.noChange} no change`);
    if (t.timedOut)
        parts.push(`${t.timedOut} timed out`);
    if (t.infraFailed)
        parts.push(`${t.infraFailed} could not run`);
    ...
    if ((r.hooks ?? []).some(h => !h.ok))
        parts.push('post-drain FAILED');
    return parts.join(' · ');
}
/**
 ...
 */
export function warnings(r) {
    const t = tally(r.results);
    const out = [];
    if (r.halted)
        out.push(r.halted);
    if (t.committed > 0) {
        out.push(`${t.committed} task(s) committed but never reached the remote — nothing else can see that ` +
            `work, and the next run may rebase over it. Check the push credentials.`);
    }
    if (t.infraFailed > 0 && t.infraFailed < t.total) {
        out.push(`${t.infraFailed} task(s) never ran (auth, usage limit or network). They were NOT blocked and ` +
            `are still queued.`);
    }
    if (t.timedOut > 0) {
    ...
    for (const h of r.hooks ?? []) {
        if (h.skipped) {
            out.push(`post-drain \`${h.command}\` did not run: ${h.skipped}`);
            continue;
        }
        if (!h.ok) {
            out.push(`post-drain \`${h.command}\` FAILED after ${h.durationS}s. The commits landed; whatever this ` +
                `command delivers did not.`);
        }
    }
    const g = groomTotals(r);
    if (g.skipped > 0) {
        out.push(`${g.skipped} task(s) could not be groomed — they need a decision a human has to make, and are ` +
...
export function renderText(r) {
    ...
    for (const w of warnings(r))
        out.push(`  ! ${w}`);
    if (warnings(r).length)
        out.push('');
...
export function renderHtml(r) {
    ...
    const warn = warnings(r).map(w => `<p style="margin:6px 0;padding:8px 10px;background:#FDF3E7;border-left:3px solid #D9A23B;font-size:14px">${esc(w)}</p>`).join('');
    return [
